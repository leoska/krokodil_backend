...
export default class WSServer extends Server {
  ...
  async init() {
    this.#server = new WebSocketServer(this.#options, () => {
      logger.info(
        `[WS-Server] Successfully started ws-server on ${this.#options.host}:${this.#options.port}`,
      );
    });

    this.#server.on('connection', (socket) => this.connection(socket, WSClient));
    this.#server.on('wsClientError', (error, socket, request) => {
      this.#wsClientError(error, socket, request);
    });
  }
