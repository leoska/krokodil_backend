...
export default class GameRoom extends EventEmitter {
  ...
  #dispatchMessageQueue() {
    if (!this.#server || typeof (this.#server.receiveBuffer) !== 'function') return;
  ...
  #dispathBufferEvents() {
    ...
    for (const idForRemove of idsForRemove) delete this.#bufferEvents[idForRemove];
  }
  ...
  async send(event, data, clientId) {
    if (!this.#bufferEvents[clientId]) this.#bufferEvents[clientId] = [];
  ...
  async sendToAll(event, data, ignoreIds = []) {
    for (const clientId of this.clientIds) {
      if (ignoreIds.includes(clientId)) continue;
